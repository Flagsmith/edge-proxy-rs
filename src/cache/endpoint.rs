...
const DEFAULT_CACHE_SIZE: NonZeroUsize = NonZeroUsize::new(128).unwrap();
...
type DocumentCache = Arc<RwLock<LruCache<CacheKey, Arc<[u8]>>>>;
...
impl EndpointCache {
    pub fn new(
        ...
        let environment_document_cache =
            if environment_document_enabled && environment_document_size > 0 {
                Some(Arc::new(RwLock::new(LruCache::new(
    ...
    pub async fn clear_environment(&self, environment_key: &str) {
        debug!(
            "Clearing endpoint caches for environment: {}",
            environment_key
        );
...
mod tests {
    ...
    async fn test_flags_cache_operations() {
        ...
        // When - fill cache to trigger LRU eviction (cache size is 2)
    ...
    async fn test_disabled_caches() {
        ...
        // When - operations should be no-ops
