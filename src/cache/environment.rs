...
use flagsmith_flag_engine::engine_eval::{environment_to_context, EngineEvaluationContext};
...
impl EnvironmentsCache for LocalMemEnvironmentsCache {
    ...
    async fn put_environment(&self, environment_key: &str, document: Value) -> bool {
        ...
        if changed {
            ...
            if let Some(overrides_array) = document.get("identity_overrides").and_then(|v| v.as_array()) {
                let mut env_identities = HashMap::new();
                for override_obj in overrides_array {
                    if let Some(identifier) = override_obj.get("identifier").and_then(|v| v.as_str()) {
