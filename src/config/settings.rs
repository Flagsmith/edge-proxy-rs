...
impl AppSettings {
    ...
    pub fn render_example_config(path: &PathBuf) -> Result<()> {
        let example = Self {
            environment_key_pairs: vec![EnvironmentKeyPair {
                server_side_key: "ser.YOUR_SERVER_KEY".to_string(),
                client_side_key: "YOUR_CLIENT_KEY".to_string(),
            }],
            api_url: default_api_url(),
            api_poll_frequency_seconds: default_api_poll_frequency(),
            api_poll_timeout_seconds: default_api_poll_timeout(),
        ...
        let json = serde_json::to_string_pretty(&example).map_err(|e| {
            EdgeProxyError::ConfigurationError(format!("Failed to serialize config: {}", e))
        })?;

        fs::write(path, json).map_err(|e| {
            EdgeProxyError::ConfigurationError(format!("Failed to write config file: {}", e))
        })?;
...
mod tests {
    ...
    fn test_client_side_key_validation_valid() {
        ...
        assert!(pair.validate_server_key().is_ok());
    }
    ...
    fn test_client_side_key_validation_invalid() {
        ...
        assert!(pair.validate_server_key().is_err());
    }
